...
@section('title', 'Crust - House | Admin - Report')

@push('styles')
    <link rel="stylesheet" href="{{ asset('CSS/Owner/report.css') }}">
...
@section('main')
    ...
    <main id="report">
        ...
        <div style="margin-top:1vw; display:flex; justify-content:space-around">
            <h3 id="branch_location">{{ $branches_name }} Branch Reports</h3>
            <div id="branch_selection">
                <select name="branch" id="branch" onchange="redirectToBranch(this)">
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ route('viewReportPage', $branch->id) }}">{{ $branch->branchName }}</option>
                    @endforeach
                </select>
            </div>
        </div>
